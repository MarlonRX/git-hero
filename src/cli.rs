...
pub fn run_cli_flow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !crate::git::is_inside_work_tree() {
        eprintln!("{}  ✖  {} (Not inside a git repository).{}", RED, crate::i18n::translate(lang, "not_git_repo"), NC);
        std::process::exit(1);
    }
    ...
    println!("{}", sep);
    println!("{}  GIT FLOW  {} {}{} {}→{} {}{}{}", BOLD, NC, CYAN, branch, BLUE, NC, CYAN, remote, NC);
    println!("{}", sep);
    println!();

    println!("{}  🔍 {}...{}", BLUE, crate::i18n::translate(lang, "status_fetching"), NC);
    ...
    println!("{}  ┌─ {}: {}/{}{}", CYAN, crate::i18n::translate(lang, "repo_status"), remote, branch, NC);
    println!("{}  ├─ {}: {}{} commit(s){}", CYAN, if lang == "es" { "Adelante" } else { "Ahead" }, GREEN, ahead, NC);
    println!("{}  └─ {}: {}{} commit(s){}", CYAN, if lang == "es" { "Detrás" } else { "Behind" }, YELLOW, behind, NC);
    ...
    if crate::git::has_uncommitted_changes() {
        ...
        if is_staged_empty {
            println!("{}  ℹ  {} (No changes staged).{}", CYAN, if lang == "es" { "Sin cambios para commit" } else { "No changes for commit" }, NC);
        } else {
            println!();
            println!("{}  💬 {}:{}", BOLD, if lang == "es" { "Mensaje del commit" } else { "Commit message" }, NC);
            print!("  {}→{} ", BLUE, NC);
            let _ = io::stdout().flush();
            ...
            if commit_msg.is_empty() {
                println!();
                println!("{}  ✖  {} (Empty message, cancelled).{}", RED, if lang == "es" { "Mensaje vacío, cancelado" } else { "Empty message, cancelled" }, NC);
                std::process::exit(1);
            }
            ...
            println!("{}  ✔  {} (Commit created).{}", GREEN, crate::i18n::translate(lang, "status_commit_success"), NC);
            did_commit = true;
        ...
        println!();
        println!("{}  ℹ  {}.{}", CYAN, if lang == "es" { "Sin cambios pendientes para commit" } else { "No pending changes for commit" }, NC);
    }

    // PULL
    if behind > 0 {
        println!();
        println!("{}  ⚠  {} (Remote is ahead by {} commit(s)).{}", YELLOW, if lang == "es" { "El remoto tiene cambios" } else { "Remote has changes" }, behind, NC);
        ...
        if confirm_action(pull_prompt) {
            ...
            println!("{}  ✔  {} (Pull completed).{}", GREEN, if lang == "es" { "Pull completado" } else { "Pull completed" }, NC);
            let new_ahead = crate::git::get_commits_ahead(&remote, &branch);
            has_unpushed = new_ahead > 0;
        }
    } else if !did_commit && !has_unpushed {
        println!();
        println!("{}  ✔  {} (Up to date with remote).{}", GREEN, if lang == "es" { "Estás al día con el remoto" } else { "You are up to date with remote" }, NC);
    }
    ...
    if has_unpushed {
        ...
        if confirm_action(push_prompt) {
            ...
            println!("{}  ✔  {} (Push completed).{}", GREEN, if lang == "es" { "Push completado" } else { "Push completed" }, NC);
        } else {
            println!("{}  📌 {} (Local commits kept, no push).{}", CYAN, if lang == "es" { "Commits locales guardados, sin push" } else { "Local commits kept, no push" }, NC);
        }
    }
